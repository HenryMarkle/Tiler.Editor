        ...
        private static void NewFrame(float dt = -1)
        {
            ...
            if (Raylib.IsWindowFullscreen())
            {
                ...
                io.DisplaySize = new Vector2(
                    Raylib.GetMonitorWidth(monitor) / dpiScale.X, Raylib.GetMonitorHeight(monitor) / dpiScale.Y);
            }
            else
            {
                io.DisplaySize = new Vector2(Raylib.GetRenderWidth() / dpiScale.X, Raylib.GetRenderHeight() / dpiScale.Y);
            }
